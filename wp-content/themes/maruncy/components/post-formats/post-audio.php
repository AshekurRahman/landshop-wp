...
<div <?php post_class(['post-box audio_post',get_post_type()]); ?>>
	<?php 
		if($maruncy_opt['post_thumbnail'] == '1'){			
            if(get_post_meta( get_the_ID(), '_maruncy_post_audio_embed', true )){
                $audio_url = get_post_meta( get_the_ID(), '_maruncy_post_audio_embed', true );
            }else{
                $audio_url = '';
            }
			if( !empty($audio_url) && function_exists('maruncy_audio_embed_content') ):
				// audio Post Content Function
				echo maruncy_audio_embed_content(esc_url($audio_url));
			else:   
