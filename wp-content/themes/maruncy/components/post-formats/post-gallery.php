...
<div <?php post_class(['post-box gallery_post'],get_post_type()); ?>>
	<?php 
		if($maruncy_opt['post_thumbnail'] == '1'){			
            if(get_post_meta( get_the_ID(), '_maruncy_post_gallery', false )){
                $gallery_images = get_post_meta( get_the_ID(), '_maruncy_post_gallery', false );
            }else{
                $gallery_images = array();
            }
            if(function_exists('maruncy_gallery_photo_list')): 
                // Gallery Post Content Function
                echo maruncy_gallery_photo_list( $gallery_images, 'full' );    
            else:
