...
<div <?php post_class(['post-box video_post'],get_post_type()); ?>>
	<?php 
		if($maruncy_opt['post_thumbnail'] == '1'){			
            if(get_post_meta( get_the_ID(), '_maruncy_post_video_embed', true )){
                $video_url = get_post_meta( get_the_ID(), '_maruncy_post_video_embed', true );
            }else{
                $video_url = '';
            }
			if( !empty($video_url) && function_exists('maruncy_video_embed_content') ):
				// Video Post Content Function
				echo maruncy_video_embed_content(esc_url($video_url));
			else:   
	...
	<div class="content">
		...
		<div class="meta_list">
			<ul>
				...
				<li class="comment">
					<span class="icon"><i class="fal fa-comment-alt-lines"></i></span>
				...
				<li class="tag">
					<span class="icon"><i class="fal fa-boxes"></i></span>
					<?php echo get_the_category_list( ' ', ' ' ); ?>
				</li>
				<?php endif; ?>
				<?php if(current_user_can('edit_posts')): ?>
				<li class="edit">
					<span class="icon"><i class="fal fa-pencil-ruler"></i></span>
					<a href="<?php echo get_edit_post_link(); ?>"><?php esc_html_e('Edit','maruncy'); ?></a>
				</li>
				<?php endif; ?>
				<?php if(function_exists('maruncy_get_post_views')): ?>
				<li class="views">
					<span class="icon"><i class="fal fa-eye"></i></span>
