 ...
 * return array
 */
function landshop_html_tag_lists() {
    $html_tag_list = [
...
function landshop_post_share_social(){
        global $post;
...
function landshop_get_post_views($postID){
...
function landshop_set_post_views($postID) {
    $count_key = 'post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    if($count==''){
        $count = 0;
 ...
 * return array
 */
function landshop_get_taxonomies( $landshop_texonomy = 'category' ){
    $terms = get_terms( array(
        'taxonomy' => $landshop_texonomy,
        'hide_empty' => true,
    ));
...
function landshop_get_title( $landshop_type = 'post' ){
    // query for your post type
    $post_type_query  = new WP_Query(  
        array (  
            'post_type'      => $landshop_type,  
            'posts_per_page' => -1  
        )  
    );   
 ...
 * return array
 */
if( !function_exists('landshop_post_name') ){
    function landshop_post_name ( $post_type = 'post', $limit = '-1' ){
        $options = array();
        $options = ['0' => esc_html__( 'None', 'landshopcore-addons' )];
 ...
 * return array
 */
function landshop_get_post_types( $args = [] ) {   
    $post_type_args = [
 ...
 * return array
 */
function landshop_elementor_template() {
    $templates = \Elementor\Plugin::instance()->templates_manager->get_source( 'local' )->get_items();
    $types = array();
    if ( empty( $templates ) ) {
        $template_lists = [ '0' => esc_html__( 'Do not Saved Templates.', 'landshopcore' ) ];
    } else {
        $template_lists = [ '0' => esc_html__( 'Select Template', 'landshopcore' ) ];
...
if(!function_exists('landshop_page_breadcrumb')){
    function landshop_page_breadcrumb( $home = 'Home', $separator = ' | ' ){
        ...
        if( is_archive() )
        {
            if (    is_category()
                ...
                $breadcrumb_trail = esc_html__( 'Author archive for ','landshopcore') .  $before . $queried_object->data->display_name . $after;
            } elseif ( is_date() ) {
        ...
        if ( is_search() ) {
            $breadcrumb_trail = esc_html__( 'Search query for: ','landshopcore' ) . $before . get_search_query() . $after;
        }
        // Handle 404's
        if ( is_404() ) {
            $breadcrumb_trail = $before . esc_html__( 'Error 404','landshopcore' ) . $after;
        }
        // Handle paged pages
        if ( is_paged() ) {
            $current_page = get_query_var( 'paged' ) ? get_query_var( 'paged' ) : get_query_var( 'page' );
            $page_addon   = $before . sprintf( esc_html__( ' ( Page %s )','landshopcore' ), number_format_i18n( $current_page ) ) . $after;
        }
