...
function nscan_activate() {
	...
	if (! current_user_can('manage_options') ) {
		exit( esc_html__('Your are not allowed to activate or deactivate plugins.', 'ninjascanner') );
	}

	global $wp_version;
	if ( version_compare( $wp_version, '4.7.0', '<' ) ) {
		exit( sprintf(
			esc_html__('NinjaScanner requires WordPress %s or greater but your current version is %s.', 'ninjascanner'),
			'4.7.0',
			esc_html( $wp_version )
		) );
	}
	...
	if ( version_compare( PHP_VERSION, '5.5', '<') ) {
		exit( sprintf(
			esc_html__('NinjaScanner requires PHP %s or greater but your current version is %s.', 'ninjascanner'),
			'5.5',
...
function nscan_init() {
	...
	if ( isset( $_GET['page'] ) && $_GET['page'] == 'NinjaScanner' && isset( $_GET['nscanop'] )
		...
		if (! preg_match( "`^($path_abs|$path_docroot)`", realpath( $file ) ) ) {
			wp_die( sprintf(
				esc_html__('File is not in the ABSPATH or DOCUMENT_ROOT: %s', 'ninjascanner'),
				esc_html( $file )
			) );
		}
...
function nscan_settings_link( $links ) {
	...
	if ( $lock_status['status'] == 'success' ) {
		$links[] = "<a href='{$get_admin_url}'>". esc_html__('A scan is running...', 'ninjascanner') .'</a>';
		unset( $links['edit'] );
		unset( $links['deactivate'] );
		return $links;
	}

	$links[] = "<a href='{$get_admin_url}'>". esc_html__('Settings', 'ninjascanner') .'</a>';
	$links[] = '<a href="https://wordpress.org/support/view/plugin-reviews/ninjascanner?'.
		'rate=5#postform" target="_blank">'. esc_html__('Rate it!', 'ninjascanner'). '</a>';
	unset( $links['edit'] );
	return $links;
}
...
function nscan_insert_jscss() {
	...
	$nscan_js_array = array(
		'cannot_start' =>
			esc_attr__('The scanning process doesn\'t seem to be able to start.', 'ninjascanner' ),
		'cancel_scan' =>
		...
		'http_error' =>
			esc_attr__('The HTTP server returned the following error:', 'ninjascanner'),
		'http_auth' =>
			esc_attr__('If your website is password-protected using HTTP basic authentication, you can enter your username and password in the "Settings > Advanced Settings" section.', 'ninjascanner'),
		'no_problem' =>
			esc_attr__('No problem detected. To refresh the list, run a new scan.', 'ninjascanner'),
		'slow_down_scan_enable' =>
			esc_attr__('Enabling this option could slow down the scanning process on low resource servers. Continue?', 'ninjascanner'),
		'restore_settings' =>
			esc_attr__('All fields will be restored to their default values. Continue?', 'ninjascanner'),
		'empty_log' =>
			esc_attr__('No records were found that match the specified search criteria.', 'ninjascanner'),
		'clear_cache_now' =>
			esc_attr__('Run the garbage collector now to clear all cached files?', 'ninjascanner'),
		'unknown_action' =>
			esc_attr__('Unknown action.', 'ninjascanner'),
		'select_elements' =>
			esc_attr__('No file selected.', 'ninjascanner'),
		'permanently_delete' =>
			esc_attr__('Permanently delete the selected files?', 'ninjascanner'),
		'restore_file' =>
			esc_attr__('Restore the selected files to their original folder?', 'ninjascanner'),
		'empty_apikey' =>
