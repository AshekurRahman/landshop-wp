 ...
 * return array
 */
function codexse_html_tag_lists() {
    $html_tag_list = [
...
function codexse_post_share_social(){
        global $post;
...
function codexse_get_post_views($postID){
...
function codexse_set_post_views($postID) {
    $count_key = 'post_views_count';
    $count = get_post_meta($postID, $count_key, true);
    if($count==''){
        $count = 0;
 ...
 * return array
 */
function codexse_get_taxonomies( $codexse_texonomy = 'category' ){
    $terms = get_terms( array(
        'taxonomy' => $codexse_texonomy,
        'hide_empty' => true,
    ));
...
function codexse_get_title( $codexse_type = 'post' ){
    // query for your post type
    $post_type_query  = new WP_Query(  
        array (  
            'post_type'      => $codexse_type,  
            'posts_per_page' => -1  
        )  
    );   
 ...
 * return array
 */
if( !function_exists('codexse_post_name') ){
    function codexse_post_name ( $post_type = 'post', $limit = '-1' ){
        $options = array();
        $options = ['0' => esc_html__( 'None', 'codexsecore-addons' )];
 ...
 * return array
 */
function codexse_get_post_types( $args = [] ) {   
    $post_type_args = [
 ...
 * return array
 */
function codexse_elementor_template() {
    $templates = \Elementor\Plugin::instance()->templates_manager->get_source( 'local' )->get_items();
    $types = array();
    if ( empty( $templates ) ) {
        $template_lists = [ '0' => esc_html__( 'Do not Saved Templates.', 'codexsecore' ) ];
    } else {
        $template_lists = [ '0' => esc_html__( 'Select Template', 'codexsecore' ) ];
...
if(!function_exists('codexse_page_breadcrumb')){
    function codexse_page_breadcrumb( $home = 'Home', $separator = ' | ' ){
        ...
        if( is_archive() )
        {
            if (    is_category()
                ...
                $breadcrumb_trail = esc_html__( 'Author archive for ','codexsecore') .  $before . $queried_object->data->display_name . $after;
            } elseif ( is_date() ) {
        ...
        if ( is_search() ) {
            $breadcrumb_trail = esc_html__( 'Search query for: ','codexsecore' ) . $before . get_search_query() . $after;
        }
        // Handle 404's
        if ( is_404() ) {
            $breadcrumb_trail = $before . esc_html__( 'Error 404','codexsecore' ) . $after;
        }
        // Handle paged pages
        if ( is_paged() ) {
            $current_page = get_query_var( 'paged' ) ? get_query_var( 'paged' ) : get_query_var( 'page' );
            $page_addon   = $before . sprintf( esc_html__( ' ( Page %s )','codexsecore' ), number_format_i18n( $current_page ) ) . $after;
        }
...
function codexse_get_current_post_type() {
...
if( !function_exists('codexse_video_embed') ){
    function codexse_video_embed($video_url){
        if( empty($video_url) ){
            return false;
        }
        wp_enqueue_style( 'codexse-post-video' );
        wp_enqueue_script( 'codexse-post-video' );
        
        $data = '<div class="post-media video-post">';
            $data .= '<div class="videoPoster" style="background-image: url('.get_the_post_thumbnail_url('','full').'); ">';
                $data .= '<button type="button" class="video-play-bttn"><i class="fa fa-play"></i></button>';
            $data .= '</div>';
            $get_embed = wp_oembed_get( esc_url($video_url) );
            $get_embed = str_replace( '?','?autoplay=1&', $get_embed );
            $data .= str_replace( 'src','data-src', $get_embed );
        $data .= '</div>';
...
if( !function_exists('codexse_audio_embed') ){
    function codexse_audio_embed($audio){
        if( empty($audio) ){
            return false;
        }
        return '<div class="post-media audio-post">'.wp_oembed_get( $audio ).'</div>';
    }
}


if( !function_exists("codexse_gallery_embed") ){
    function codexse_gallery_embed( $gallery_images, $img_size = 'large' ) {
        if( empty($gallery_images) ){
            return false;
        }
        wp_enqueue_script( 'swiper-slider' );
        wp_enqueue_style( 'swiper-slider' );
        wp_enqueue_script( 'codexse-post-gallery' );
        wp_enqueue_style( 'codexse-post-gallery' );
...
*/
function codexse_is_elementor_version( $operator = '<', $version = '2.6.0' ) {
