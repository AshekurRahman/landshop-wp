...
class PDF_View extends Base {
    ...
    public function get_title() {
        return __('PDF View', 'codexse-elementor-addons');
    }

    public function get_custom_help_url() {
        return 'https://codexseaddons.com/docs/codexse-addons-for-elementor/widgets/pdf-view/';
    }
    ...
    protected function __pdf_content_controls() {
        $this->start_controls_section(
            ...
            [
                'label' => __('PDF Source', 'codexse-elementor-addons'),
		...
		$this->add_control(
			'file_type',
			[
				'label' => __('File Source', 'codexse-elementor-addons'),
				'type' => Controls_Manager::SELECT,
				'options' =>[
					'url' => __('URL', 'codexse-elementor-addons'),
					'upload_file' => __('Upload File', 'codexse-elementor-addons'),
				],
                'default' => 'url'
			]
		);
		...
		$this->add_control(
			'pdf_url',
			[
				'label' => __('PDF URL', 'codexse-elementor-addons'),
				'type' => Controls_Manager::URL,
				'placeholder' => __( 'http://www.example.com/sample.pdf', 'codexse-elementor-addons'),
                'default' => [
                    'url' =>  'https://demo.codexseaddons.com/wp-content/uploads/2022/03/file-sample.pdf'
                ],
				'show_external' => false,
				'dynamic' => [
					'active' => true,
		...
		$this->add_control(
			'pdf_file',
			[
				'label' => __( 'Choose PDF',  'codexse-elementor-addons' ),
				'type' => Controls_Manager::MEDIA,
				'media_type' => 'application/pdf',
                'default' => [
                    'url' => CODEXSE_ADDONS_ASSETS . 'vendor/pdfjs/sample.pdf'
                ],
				'dynamic' => [
					'active' => true,
				],
		...
		$this->add_control(
			'page_number',
			[
				'label' => __( 'Page Number', 'codexse-elementor-addons' ),
        ...
        $this->add_control(
			'pdf_title',
			[
				'label' => __( 'PDF Title', 'codexse-elementor-addons' ),
				'type' => Controls_Manager::TEXT,
				'label_block' => true,
				'default' => __( 'PDF Title', 'codexse-elementor-addons' ),
				'placeholder' => __( 'Type PDF title', 'codexse-elementor-addons' ),
				'separator' => 'before',
				'dynamic' => [
					'active' => true,
				]
	...
	protected function __pdf_settings_controls(){
		...
		$this->add_control(
			'enable_download',
			[
				'label'        => __( 'Show Download?', 'codexse-elementor-addons' ),
        ...
        $this->add_control(
			'enable_icon',
			[
				'label'        => __( 'Show Icon?', 'codexse-elementor-addons' ),
				'type'         => Controls_Manager::SWITCHER,
				'default'      => 'yes',
				'return_value' => 'yes',
			]
    ...
    protected function __title_desc_style_controls() {
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .cx-pdf-title' => 'color: {{VALUE}}',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'label' => __( 'Title Bar', 'codexse-elementor-addons' ),
				'separator' => 'before'
			]
		);
    ...
    protected function render() {
		...
		if(empty($pdf_url_i)){
			$pdf_url_i = CODEXSE_ADDONS_ASSETS . 'vendor/pdfjs/sample.pdf';
		}
        ...
        <div class="pdf_viewer_container">
            <div class="pdf_viewer_options <?php echo esc_attr($unique_id); ?>" <?php $this->print_render_attribute_string( 'pdf_settings' ) ?>>
				<span class="cx-title-flex">
					<span class="pdf-icon">
						<?php Icons_Manager::render_icon($settings['icon'], ['aria-hidden' => 'true']);  ?>
					</span>
					<?php
					if($settings['pdf_title']){
						echo sprintf( '<h2 class="cx-pdf-title">%s</h2>',
                ...
                <?php
                    if('yes' == $settings['enable_download']){
                        printf( '<a href="%1$s" class="cx-btn" download title="%2$s">%3$s</a>',
                            esc_url($pdf_url_i),
                            esc_html( $settings['pdf_title'] ),
                            __('Download', 'codexse-elementor-addons')
                        );
                    }
                ?>
                </div>
