...
class Image_Stack_Group extends Base {
	...
	public function get_title() {
		return __( 'Image Stack Group', 'codexse-elementor-addons' );
	}

	public function get_custom_help_url() {
		return 'https://codexseaddons.com/docs/codexse-addons-for-elementor/widgets/image-stack-group/';
	}
	...
	protected function register_content_controls() {
		...
		$repeater->add_control(
			'tooltip_position',
			[
				'label' => __( 'Tooltip Position', 'codexse-elementor-addons' ),
				...
				'options' => [
					...
					'up'  => [
						'title' => __( 'Up', 'codexse-elementor-addons' ),
						'icon' => 'eicon-v-align-top'
					],
					'down'  => [
						'title' => __( 'Down', 'codexse-elementor-addons' ),
		...
		$repeater->add_control(
			...
			[
				...
				'condition' => [ 'media_type' => 'icon' ],
				'selectors' => [
					'{{WRAPPER}} .cx-cig-item{{CURRENT_ITEM}} i' => 'color: {{VALUE}}'
				]
			]
		);
		...
		$repeater->add_group_control(
			...
			[
				...
				'exclude' => [ 'image' ],
				'selector' => '{{WRAPPER}} .cx-cig-item{{CURRENT_ITEM}} i'
			]
		);
	...
	protected function register_style_controls() {
		$this->start_controls_section(
			'_section_style_icon',
			[
				'label' => __( 'Image / Icon', 'codexse-elementor-addons' ),
		...
		$this->add_responsive_control(
			'img_size',
			[
				'label' => __( 'Item Size', 'codexse-elementor-addons' ),
				...
				'selectors' => [
					'{{WRAPPER}} .cx-cig-item i,{{WRAPPER}} .cx-cig-item img' => 'width: {{SIZE}}{{UNIT}}; height: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .cx-cig-item i,{{WRAPPER}} .cx-cig-item .fw-svg-wrap' => 'width: {{SIZE}}{{UNIT}}; height: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .cx-cig-item i' => 'font-size: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .cx-cig-item svg' => 'width: {{SIZE}}{{UNIT}}; height: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .cx-cig-item:not(:last-child)' => 'margin-right: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_responsive_control(
			'icon_border_size',
			[
				'label' => __( 'Border Size', 'codexse-elementor-addons' ),
				'type' => Controls_Manager::SLIDER,
				'size_units' => ['px'],
				'default' => [
					'unit' => 'px',
				...
				'selectors' => [
					'{{WRAPPER}} .cx-cig-item i,{{WRAPPER}} .cx-cig-item img,{{WRAPPER}} .cx-cig-item .fw-svg-wrap' => 'border-width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .cx-cig-item i' => 'border-color: {{VALUE}};',
					'{{WRAPPER}} .cx-cig-item img' => 'border-color: {{VALUE}};',
					'{{WRAPPER}} .cx-cig-item .fw-svg-wrap' => 'border-color: {{VALUE}};',
				],
			]
		);
		...
		$this->add_responsive_control(
            ...
            [
                ...
                'size_units' => [ 'px', '%' ],
                'selectors' => [
                    '{{WRAPPER}} .cx-cig-item,{{WRAPPER}}  .cx-cig-item i, {{WRAPPER}} .cx-cig-item img, {{WRAPPER}} .cx-cig-item .fw-svg-wrap' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
            ]
		);
		...
		$this->add_control(
			...
			[
				...
				'type' => Controls_Manager::COLOR,
				'selectors' => [
					'{{WRAPPER}} .cx-cig-item i' => 'color: {{VALUE}}'
				]
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'types' => [ 'classic', 'gradient' ],
				'exclude' => [ 'image' ],
				'selector' => '{{WRAPPER}} .cx-cig-item i'
			]
		);
		...
		$this->add_group_control( Group_Control_Box_Shadow::get_type(),
			[
				...
				'selector' => '{{WRAPPER}} .cx-cig-item-outline',
			]
		);
		...
		$this->start_controls_section(
			...
			[
				'label' => __( 'Tooltips', 'codexse-elementor-addons' ),
				'tab'   => Controls_Manager::TAB_STYLE,
			]
		);
	...
	protected function render() {
		...
		$fs_inline_fw = cx_elementor()->experiments->is_feature_active( 'e_font_icon_svg' );
		?>
		...
		<div class="cx-cig">
			<?php foreach ( $settings['images'] as $item ) :
				...
				if($media_type == "icon"){
					...
					ob_start();
					cx_render_icon( $item, 'icon', 'selected_icon', $attr);
				...
				$id = 'cx-cig-item-' . $item['_id'];

				$link = $item['link'];

				if(!empty($link['url'])){
					$this->add_link_attributes( $id, $item['link'] );
					$wrap_start = '<a '.$this->get_render_attribute_string( $id ).' class="cx-cig-item cx-cig-item-outline '.$item_id.'" '.$tooltip_data.'>';
					$wrap_end   = '</a>';
				}else{
					$wrap_start = '<span class="cx-cig-item cx-cig-item-outline '.$item_id.'" '.$tooltip_data.'>';
					$wrap_end   = '</span>';
				}
