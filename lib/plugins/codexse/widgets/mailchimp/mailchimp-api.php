...
namespace Codexse_Addons\Elementor\Widget\Mailchimp;
...
class Mailchimp_Api {
	...
	public static function insert_subscriber_to_mailchimp( $submitted_data ) {
		...
		self::$credentials = cx_get_credentials( 'mailchimp' );

		self::$api_key = isset( self::$credentials['api'] ) ? self::$credentials['api'] : '';

		$widget_settings = cx_get_ele_widget_settings( absint($_POST['post_id']), sanitize_text_field($_POST['widget_id']) );
		...
		if ( ! isset( $server[1] ) ) {
			return ['status' => 0, 'msg' => esc_html__( 'Invalid API key.', 'codexse-elementor-addons' )];
		}
		...
		if ( is_wp_error( $response ) ) {
			...
			if ( $body['status'] > 399 && $body['status'] < 600 ) {
				...
				$return['status'] = 1;
				$return['msg']    = esc_html__( 'Confirm your subscription from your email.', 'codexse-elementor-addons' );
			} else {
				$return['status'] = 0;
				$return['msg']    = esc_html__( 'Something went wrong. Try again later.', 'codexse-elementor-addons' );
			}
		}
