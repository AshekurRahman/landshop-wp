...
class Ajax_Handler {
	...
	public static function init() {
		...
		add_action( 'wp_ajax_cx_twitter_feed_action', [ __CLASS__, 'twitter_feed_ajax' ] );
		add_action( 'wp_ajax_nopriv_cx_twitter_feed_action', [ __CLASS__, 'twitter_feed_ajax' ] );

		add_action( 'wp_ajax_cx_post_tab_action', [ __CLASS__, 'post_tab' ] );
		add_action( 'wp_ajax_nopriv_cx_post_tab_action', [ __CLASS__, 'post_tab' ] );

		add_action( 'wp_ajax_cx_mailchimp_ajax', [__CLASS__, 'mailchimp_prepare_ajax'] );
		add_action( 'wp_ajax_nopriv_cx_mailchimp_ajax', [__CLASS__, 'mailchimp_prepare_ajax'] );
	...
	*/
	public static function twitter_feed_ajax() {
		...
		$security = check_ajax_referer( 'codexse_addons_nonce', 'security' );

		if ( true == $security && isset( $_POST['query_settings'] ) ) :
			$settings    = cx_sanitize_array_recursively($_POST['query_settings']);
			$loaded_item = absint($_POST['loaded_item']);

			$user_name      = trim( $settings['user_name'] );
			$cx_tweets_cash = '_' . $settings['id'] . '_tweet_cash';
						...
						<div class="cx-tweet-author">
							<?php if ( 'yes' == $settings['show_user_image'] ) : ?>
								<a href="<?php echo esc_url( 'https://twitter.com/' . $user_name ); ?>">
									<img src="<?php echo esc_url( $item['user']['profile_image_url_https'] ); ?>" alt="<?php echo esc_attr( $item['user']['name'] ); ?>" class="cx-tweet-avatar">
								</a>
							<?php endif; ?>

							<div class="cx-tweet-user">
								<?php if ( 'yes' == $settings['show_name'] ) : ?>
									<a href="<?php echo esc_url( 'https://twitter.com/' . $user_name ); ?>" class="cx-tweet-author-name">
										<?php echo esc_html( $item['user']['name'] ); ?>
									</a>
								<?php endif; ?>

								<?php if ( 'yes' == $settings['show_user_name'] ) : ?>
									<a href="<?php echo esc_url( 'https://twitter.com/' . $user_name ); ?>" class="cx-tweet-username">
	...
	public static function post_tab() {
		...
		if ( true == $security ) :
			$settings   = cx_sanitize_array_recursively($_POST['post_tab_query']);
			...
			if ( count( $posts ) !== 0 ) :
				?>
				<div class="cx-post-tab-item-wrapper active" data-term="<?php echo esc_attr( $term_id ); ?>">
					<?php foreach ( $posts as $post ) : ?>
						<div class="cx-post-tab-item">
							<div class="cx-post-tab-item-inner">
								<?php if ( has_post_thumbnail( $post->ID ) ) : ?>
									<a href="<?php echo esc_url( get_the_permalink( $post->ID ) ); ?>" class="cx-post-tab-thumb">
	...
	public static function mailchimp_prepare_ajax() {
		...
		parse_str( isset( $_POST['subscriber_info'] ) ? cx_sanitize_array_recursively($_POST['subscriber_info']) : '', $subsciber );

		if ( ! class_exists( 'Codexse_Addons\Elementor\Widget\Mailchimp\Mailchimp_Api' ) ) {
			include_once CODEXSE_ADDONS_DIR_PATH . 'widgets/mailchimp/mailchimp-api.php';
		}
