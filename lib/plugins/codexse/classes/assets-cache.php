...
class Assets_Cache {
	...
	const FILE_PREFIX = 'cx-';

	const BASE_DIR = 'codexseaddons';
	...
	public function enqueue() {
		...
		if ( $this->has() ) {
			wp_enqueue_style(
				'codexse-elementor-addons-' . $this->get_post_id(),
				$this->get_file_url(),
				[ 'elementor-frontend' ],
				CODEXSE_ADDONS_VERSION . '.' . get_post_modified_time()
			);
		}
	}
	...
	protected function get_css( $files_name, $is_pro = false ) {
		...
		foreach ( $files_name as $file_name ) {
			$file_path = CODEXSE_ADDONS_DIR_PATH . "assets/css/widgets/{$file_name}.min.css";
			$file_path = apply_filters( 'codexseaddons_get_styles_file_path', $file_path, $file_name, $is_pro );
