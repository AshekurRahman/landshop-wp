...
class Cache_Manager {
	...
	public static function is_editing_mode() {
		return (
			cx_elementor()->editor->is_edit_mode() ||
			cx_elementor()->preview->is_preview_mode() ||
			is_preview()
		);
	}
	...
	public static function is_built_with_elementor( $post_id ) {
		...
		if(!empty($post) && isset($post->ID)) {
			// return cx_elementor()->db->is_built_with_elementor( $post_id );
			return cx_elementor()->documents->get( $post->ID )->is_built_with_elementor();
		}
	...
	public static function enqueue_fa5_fonts( $post_id ) {
		...
		if ( ! empty( $meta['icons'] ) ) {
			...
			foreach ( $meta['icons'] as $icon_font ) {
				...
				cx_elementor()->frontend->enqueue_font( $icon_font );
			}
		}
	}
	...
	public static function enqueue( $post_id ) {
		...
		wp_enqueue_script( 'codexse-elementor-addons' );

		do_action( 'codexseaddons_enqueue_assets', $is_cache = true, $post_id );
	}
	...
	public static function enqueue_raw() {
		...
		wp_enqueue_style( 'codexse-elementor-addons' );
		wp_enqueue_script( 'codexse-elementor-addons' );

		do_action( 'codexseaddons_enqueue_assets', $is_cache = false, 0 );
	}
}
