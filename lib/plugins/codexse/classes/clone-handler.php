...
class Clone_Handler {
	 ...
	 */
	const ACTION = 'cx_duplicate_thing';
	...
	public static function add_row_actions( $actions, $post ) {
		if ( current_user_can( 'edit_post', $post->ID ) && post_type_supports( $post->post_type, 'elementor' ) ) {
			$actions[ self::ACTION ] = sprintf(
				...
				sprintf( esc_attr__( 'Clone - %s', 'codexse-elementor-addons' ), esc_attr( $post->post_title ) ),
				esc_html__( 'Codexse Clone', 'codexse-elementor-addons' )
			);
		}
	...
	public static function duplicate_thing() {
		...
		if ( ! is_wp_error( $duplicated_post_id ) ) {
			...
			if ( $ref === 'editor' ) {
				$document = cx_elementor()->documents->get( $duplicated_post_id );
	...
	protected static function duplicate_post( $post ) {
		...
		$duplicated_post_args = [
			...
			'post_title'     => sprintf( __( '%s - [Cloned #%d]', 'codexse-elementor-addons' ), $post->post_title,
				$post->ID ),
		];
