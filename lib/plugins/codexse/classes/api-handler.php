...
class Api_Handler
{
    ...
    const CONSENT_DB_KEY = 'codexse-elementor-addons_allow_tracking'; 
    const CACHE_DB_KEY = 'codexse-elementor-addons_wizard_cache_key'; 
    const WIZARD_CACHE_FIX  = "codexse-elementor-x98237938759348573";

    public static function init()
    {
        // need to delete after 1/2 release
        if(get_option('codexse-elementor-addons_wizard_cache')){
            delete_option('codexse-elementor-addons_wizard_cache');
        }

        add_action('rest_api_init', [__CLASS__, 'cx_wizard_routes']);
    ...
    public static function cx_wizard_routes()
    {
        /* Get Ends */
        register_rest_route('codexse/v1', '/wizard/dummy', array(
            'methods' => 'GET',
            'callback' => [__CLASS__,'cx_wizard_get_dummy_data'],
            'permission_callback' => function () {
                return true;
            }
        ));

        register_rest_route('codexse/v1', '/wizard/cache', array(
            'methods' => 'GET',
            'callback' => [__CLASS__,'cx_wizard_get_cache_data'],
            'permission_callback' => function () {
                return current_user_can( 'edit_others_posts' );
            }
        ));

        register_rest_route('codexse/v1', '/wizard/preset/normal', array(
            'methods' => 'GET',
            'callback' => [__CLASS__,'cx_wizard_get_preset'],
            'permission_callback' => function () {
                return current_user_can( 'edit_others_posts' );
            }
        ));
        ...
        register_rest_route('codexse/v1', '/wizard/save', array(
            'methods' => 'POST',
            'callback' => [__CLASS__,'cx_wizard_save_data'],
            'permission_callback' => function () {
                return current_user_can( 'edit_others_posts' );
            }
        ));

        register_rest_route('codexse/v1', '/wizard/save-cache', array(
            'methods' => 'POST',
            'callback' => [__CLASS__,'cx_wizard_save_cache_data'],
            'permission_callback' => function () {
                return current_user_can( 'edit_others_posts' );
            }
        ));

        register_rest_route('codexse/v1', '/wizard/skip', array(
            'methods' => 'POST',
            'callback' => [__CLASS__,'cx_wizard_skip'],
            'permission_callback' => function () {
                return current_user_can( 'edit_others_posts' );
            }
        ));
    ...
    public static function cx_wizard_get_dummy_data(){
    ...
    public static function cx_wizard_get_preset(){
    ...
    public static function cx_wizard_save_data(\WP_REST_Request $request){
        ...
        if($data){
            ...
            update_option(CODEXSE_ADDONS_WIZARD_REDIRECTION_FLAG,1);
            delete_option(self::CACHE_DB_KEY);
        }
    ...
    public static function cx_wizard_skip(\WP_REST_Request $request){
        
        $stat = update_option(CODEXSE_ADDONS_WIZARD_REDIRECTION_FLAG,1);
    ...
    public static function cx_wizard_save_cache_data(\WP_REST_Request $request){
    ...
    public static function cx_wizard_get_cache_data(){
    ...
    public static function cx_wizard_get_widgets(){
