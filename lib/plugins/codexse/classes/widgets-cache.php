...
class Widgets_Cache {
    ...
    const OPTION_KEY = 'codexseaddons_elements_cache';

    const META_KEY = '_codexseaddons_elements_cache';
    ...
    protected function get_global_widget_type( $template_id ) {
        $template_data = cx_elementor()->templates_manager->get_template_data( [
            'source' => 'local',
        ...
        $original_widget_type = cx_elementor()->widgets_manager->get_widget_types( $template_data['content'][0]['widgetType'] );
    ...
    public function get() {
        ...
        return array_map( function( $widget_key ) {
            return str_replace( 'cx-', '', $widget_key );
        }, array_keys( $cache ) );
    }
    ...
    public function get_elementor_data() {
        ...
        if ( is_null( $this->elementor_data ) ) {
            $document = cx_elementor()->documents->get( $this->get_post_id() );
    ...
    public function save() {
        ...
        $cache = [];
        cx_elementor()->db->iterate_data( $data, function ( $element ) use ( &$cache ) {
            $type = $this->get_widget_type( $element );

            if ( strpos( $type, 'cx-' ) !== false ) {
                if ( ! isset( $cache[ $type ] ) ) {
                    $cache[ $type ] = 0;
                }
                $cache[ $type ] ++;
