...
class Library_Source extends Source_Base {
	 ...
	 */
	const LIBRARY_CACHE_KEY = 'cx_library_cache';
	...
	public function get_title() {
		return __( 'Codexse Library', 'codexse-elementor-addons' );
	}
	...
	public static function request_template_data( $template_id ) {
		...
		$body = [
			...
			'version' => CODEXSE_ADDONS_VERSION,
		];

		if ( cx_has_pro() ) {
			$body['has_pro'] = 1;
			$body['pro_version'] = CODEXSE_ADDONS_PRO_VERSION;
		}
	...
	public function get_data( array $args, $context = 'display' ) {
		...
		if ( empty( $data ) || empty( $data['content'] ) ) {
			throw new \Exception( __( 'Template does not have any content', 'codexse-elementor-addons' ) );
		}
		...
		$document = cx_elementor()->documents->get( $post_id );
