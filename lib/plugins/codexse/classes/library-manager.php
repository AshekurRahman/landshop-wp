...
class Library_Manager {
	...
	public static function print_template_views() {
		include_once CODEXSE_ADDONS_DIR_PATH . 'templates/template-library/templates.php';
	}

	public static function enqueue_assets() {
		wp_enqueue_style(
			'codexse-addons-templates-library',
			CODEXSE_ADDONS_ASSETS . 'admin/css/template-library.min.css',
			[
				'elementor-editor',
			],
		...
		wp_enqueue_script(
			'codexse-addons-templates-library',
			CODEXSE_ADDONS_ASSETS . 'admin/js/template-library.min.js',
			[
	...
	public static function register_ajax_actions( Ajax $ajax ) {
		$ajax->register_ajax_action( 'get_cx_library_data', function( $data ) {
			...
			if ( ! empty( $data['editor_post_id'] ) ) {
				...
				if ( ! get_post( $editor_post_id ) ) {
					throw new \Exception( __( 'Post not found.', 'codexse-elementor-addons' ) );
				}

				cx_elementor()->db->switch_to_post( $editor_post_id );
			}
		...
		$ajax->register_ajax_action( 'get_cx_template_data', function( $data ) {
			if ( ! current_user_can( 'edit_posts' ) ) {
				throw new \Exception( 'Access Denied' );
			}
			...
			if ( empty( $data['template_id'] ) ) {
				throw new \Exception( __( 'Template id missing', 'codexse-elementor-addons' ) );
			}
