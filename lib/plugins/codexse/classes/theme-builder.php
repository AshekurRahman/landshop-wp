...
use Codexse_Addons\Elementor\Conditions_Cache;
...
class Theme_Builder {
    ...
    public function __construct() {
        ...
        add_action('elementor/editor/after_enqueue_scripts', [$this, 'cx_template_element_scripts']);
        add_action('elementor/editor/after_enqueue_scripts', [$this, 'edit_template_condition_modal'], 10, 2);

        add_filter('elementor/document/config', [$this, 'cx_template_document_title'], 10, 2);

        // Admin Actions
        add_action('admin_action_cx_library_new_post', [$this, 'admin_action_new_post']);

        add_action('current_screen', function () {
            $current_screen = get_current_screen();
            if (!$current_screen || !strstr($current_screen->post_type, 'cx_library')) {
                return;
            }
        ...
        add_filter('template_include', [$this, 'cx_theme_builder_content'], 999);
        add_action('codexseaddons_theme_builder_render', array($this, 'single_blog_content_elementor'), 999);

        // Register Ajax Handles
        // add_action( 'wp_ajax_cx_cond_template_type', [$this, 'cx_get_template_type'] );
    }

    public function add_query_vars_filter($vars) {
        $vars[] = "cx_library_type";
        return $vars;
    }
    ...
    public function create_themebuilder_cpt() {
        ...
        $labels = array(
            'name' => _x('Theme Builder', 'Post Type General Name', 'codexse-elementor-addons'),
            'singular_name' => _x('Theme Builder', 'Post Type Singular Name', 'codexse-elementor-addons'),
            'menu_name' => _x('Theme Builder', 'Admin Menu text', 'codexse-elementor-addons'),
            'name_admin_bar' => _x('Theme Builder', 'Add New on Toolbar', 'codexse-elementor-addons'),
            'archives' => __('Theme Builder Archives', 'codexse-elementor-addons'),
            'attributes' => __('Theme Builder Attributes', 'codexse-elementor-addons'),
            'parent_item_colon' => __('Parent Theme Builder:', 'codexse-elementor-addons'),
            'all_items' => __('All Theme Builder', 'codexse-elementor-addons'),
            'add_new_item' => __('Add New Theme Builder', 'codexse-elementor-addons'),
            'add_new' => __('Add New', 'codexse-elementor-addons'),
            'new_item' => __('New Theme Builder', 'codexse-elementor-addons'),
            'edit_item' => __('Edit Theme Builder', 'codexse-elementor-addons'),
            'update_item' => __('Update Theme Builder', 'codexse-elementor-addons'),
            'view_item' => __('View Theme Builder', 'codexse-elementor-addons'),
            'view_items' => __('View Theme Builder', 'codexse-elementor-addons'),
            'search_items' => __('Search Theme Builder', 'codexse-elementor-addons'),
            'not_found' => __('Not found', 'codexse-elementor-addons'),
            'not_found_in_trash' => __('Not found in Trash', 'codexse-elementor-addons'),
            'featured_image' => __('Featured Image', 'codexse-elementor-addons'),
            'set_featured_image' => __('Set featured image', 'codexse-elementor-addons'),
            'remove_featured_image' => __('Remove featured image', 'codexse-elementor-addons'),
            'use_featured_image' => __('Use as featured image', 'codexse-elementor-addons'),
            'insert_into_item' => __('Insert into Theme Builder', 'codexse-elementor-addons'),
            'uploaded_to_this_item' => __('Uploaded to this Theme Builder', 'codexse-elementor-addons'),
            'items_list' => __('Theme Builder list', 'codexse-elementor-addons'),
            'items_list_navigation' => __('Theme Builder list navigation', 'codexse-elementor-addons'),
            'filter_items_list' => __('Filter Theme Builder list', 'codexse-elementor-addons'),
        );
        $args = array(
            'label' => __('Theme Builder', 'codexse-elementor-addons'),
            'description' => __('', 'codexse-elementor-addons'),
            'labels' => $labels,
        ...
        register_post_type('cx_library', $args);
    }
    ...
    public function modify_menu() {
        add_submenu_page(
            ...
            'edit.php?post_type=cx_library',  // Slug
            false // Function
        );
    }
    ...
    public function add_role_filter_to_posts_query($query) {
        ...
        if ('edit.php' === $pagenow && self::CPT === $query->query['post_type']) {
            ...
            if (isset($_GET['cx_library_type'])) {
                $meta_query = array(
                    array(
                        'key' => '_cx_library_type',
                        'value' => sanitize_text_field( $_GET['cx_library_type']),
                        'compare' => '=='
                    )
                );
                $query->set('meta_query', $meta_query);
                $query->set('meta_key', '_cx_library_type');
            }
        }
    }
...
?>
        <div id="cx-admin-top-bar-root">
            <div class="cx-admin-top-bar">
                <div class="cx-admin-top-bar__main-area">
                    <div class="cx-admin-top-bar__heading">
                        <div class="cx-admin-top-bar__heading-logo">
                            <svg width="30" height="36" viewBox="0 0 30 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M30 30.5126C26.8157 33.4779 22.526 35.2941 17.811 35.2941C7.97244 35.2941 0 27.3927 0 17.6471C0 7.90139 7.97244 0 17.811 0C22.3913 0 26.5701 1.71322 29.7236 4.52879L20.915 13.7736L15.7016 8.29693H8.92205L17.5913 17.5183L8.5748 27.1797H15.078L20.915 21.0618L29.7661 30.5126H30Z" fill="white"/>
                            </svg>
                        </div>
                        <h1 class="cx-admin-top-bar__heading-title">Theme Builder</h1>
                    </div>
                    <div class="cx-admin-top-bar__main-area-buttons">
                        <a class="page-title-action" id="cx-template-library-add-new" href="http://ha.test/wp-admin/post-new.php?post_type=cx_library">Add New</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    public static function admin_columns_headers($posts_columns) {
        ...
        $posts_columns = array_slice($posts_columns, 0, $offset, true) + [
            'type' => __('Type', 'codexse-elementor-addons'),
            'condition' => __('Conditions', 'codexse-elementor-addons'),
    ...
    public function admin_columns_content( $column_name, $post_id ) {
        ...
        if ('type' === $column_name) {
            ...
            $type       = get_post_meta($post_id, '_cx_library_type', true);
            $isActive   = get_post_meta($post_id, '_cx_template_active', true);
    ...
    public function admin_print_tabs($views) {
        $getActive = get_query_var('cx_library_type');
        // var_dump($getActive);
    ?>
        <div id="codexseaddon-template-library-tabs-wrapper" class="nav-tab-wrapper">
            ...
            foreach (self::TEMPLATE_TYPE as $key => $value) {
                ...
                $admin_filter_url = admin_url(self::TAB_BASE . '&cx_library_type=' . $key);
...
<?php
    ...
    public function add_new_template_template() {
        ob_start();
        include(CODEXSE_ADDONS_DIR_PATH . 'templates/admin/new-template.php');
    ...
    //     ob_start();
    //     include(CODEXSE_ADDONS_DIR_PATH . 'templates/admin/edit-template.php');
    //     $template = ob_get_clean();
    //     echo $template;
    // }
    ...
    public function edit_template_condition_modal() {
        if (self::CPT === get_post_type()) {
            ob_start();
            include(CODEXSE_ADDONS_DIR_PATH . 'templates/admin/edit-template-condition.php');
            $template = ob_get_clean();
            echo $template;
        }
    }
    ...
    public function admin_action_new_post() {
        ...
        check_admin_referer('cx_library_new_post_action');
        ...
        $post_data = isset($_GET['post_data']) ? cx_sanitize_array_recursively($_GET['post_data']) : [];
    ...
    protected function create_template_document($type, $post_data, $meta) {
        if (empty($post_data['post_title'])) {
            $post_data['post_title'] = esc_html__('Elementor', 'codexse-elementor-addons');
            $update_title = true;
        }
        ...
        $meta_data['_cx_library_type']  = $type;
        $meta_data['_cx_display_cond']  = $meta['display_conditions'];
    ...
    public static function template_ids() {
        $cached = wp_cache_get('cx_template_ids');
        if (false !== $cached) {
            return $cached;
        }
        ...
        wp_cache_set('cx_template_ids', $ids);
        return $ids;
    }
    ...
    public function get_document_conditions($post_id) {
        $saved_conditions = get_post_meta($post_id, '_cx_display_cond', true);
    ...
    public function cx_theme_builder_content($template) {
        $location = '';
                ...
                if ($theme_document) {
                    switch ($templateType) {
                        case "elementor_canvas":
                            $template = CODEXSE_ADDONS_DIR_PATH . 'templates/builder/singular/canvas.php';
                            break;
                        case "elementor_header_footer":
                            $template = CODEXSE_ADDONS_DIR_PATH . 'templates/builder/singular/fullwidth.php';
                            break;
                        default:
                            // $template = $template;
                            $template = CODEXSE_ADDONS_DIR_PATH . 'templates/builder/singular/fullwidth.php';
    ...
    */
    ...
    protected function get_full_data($post) {
        if ($post != null) {
            $tpl_type = get_post_meta($post->ID, '_cx_library_type', true);
            $tpl_cond = get_post_meta($post->ID, '_cx_display_cond', true);
    ...
    function cx_template_element_scripts() {
        if (self::CPT === get_post_type()) {
            wp_enqueue_script(
                'codexse-addons-template-elements',
                CODEXSE_ADDONS_ASSETS . 'admin/js/template-elements.min.js',
                ['jquery', 'codexse-elementor-addons-editor'],
                CODEXSE_ADDONS_VERSION,
                true
            );

            wp_enqueue_script(
                'codexse-addons-micromodal',
    ...
    public function cx_template_document_title($config, $post_id) {
        $tpl_type = get_post_meta($post_id, '_cx_library_type', true);
