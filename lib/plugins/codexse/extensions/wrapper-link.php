...
class Wrapper_Link {
	...
	public static function add_controls_section( Element_Base $element) {
		...
		$element->start_controls_section(
			'_section_cx_wrapper_link',
			[
				'label' => __( 'Wrapper Link', 'codexse-elementor-addons' ) . cx_get_section_icon(),
				'tab'   => $tabs,
			]
		);
	...
	public static function before_section_render( Element_Base $element ) {
		$link_settings = $element->get_settings_for_display( 'cx_element_link' );
		...
		if ( $link_settings && ! empty( $link_settings['url'] ) ) {
			$element->add_render_attribute(
				'_wrapper',
				[
					'data-cx-element-link' => json_encode( $link_settings ),
