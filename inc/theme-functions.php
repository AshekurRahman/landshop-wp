    ...
    function landshop_post_thumbnail( $thumb_size = 'large' ) {
        ...
        if ( is_single() ) {
           ...
           printf( '<figure class="post__media">%1$s</figure>',get_the_post_thumbnail( '', $thumb_size )); 
            ...
            printf( '<figure class="post__media" ><a href="%1$s" aria-hidden="true">%2$s</a></figure>', get_the_permalink(), get_the_post_thumbnail( '', $thumb_size ));
        }
    }
endif;
...
function landshop_mainmenu_demo_content(){ 
    ...
    printf( wp_kses('<a class="primary__button" href="%s">%s</a>',wp_kses_allowed_html('post')), esc_url(admin_url('nav-menus.php')), esc_html__('Setup a menu','landshop'));
}



function landshop_get_google_fonts() {
    $font_api_url = 'https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyDA2fUYStGwSU009RGHYAE4DkAOkjVMU3w';
    $response = wp_remote_get($font_api_url);
    $fonts = array();

    if (!is_wp_error($response)) {
        $body = wp_remote_retrieve_body($response);
        $fonts = json_decode($body)->items;
    }

    // Create choices array for font selection
    $choices = array();
